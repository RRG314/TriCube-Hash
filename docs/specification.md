...
This document specifies the current public TriCube baseline construction as
implemented in `c/src/tricube.c`. It is written to make the primitive readable
and reimplementable. It is not a security proof and it is not a standard.
...
baseline construction unless a variant is explicitly named.
...
The black-box differential probe flips selected input differences and measures
output Hamming distance, output-bit bias, and repeated output differences across
selected reduced-round settings. It does not model differential propagation
inside the round function. It does not search differential trails, compute
maximum differential probability, or replace differential cryptanalysis.

The black-box rotational probe applies selected word rotations and measures
rotational distance and exact preserved-relation counts across selected
reduced-round settings. It does not prove resistance to rotational
distinguishers.

The small black-box algebraic screen samples selected variables and output bits
and estimates algebraic-normal-form degree under a limited budget. It does not
perform SAT, MILP, Gröbner-basis, full ANF, invariant, or integral analysis.

The state-recovery screen measures simple predictor behavior against random
baselines. It does not model or recover the internal 2048-bit state.

The overlap/fork screen checks for repeated 32-byte blocks across related
streams. It does not prove stream independence.
