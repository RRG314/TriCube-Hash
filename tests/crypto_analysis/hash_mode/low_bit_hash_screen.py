#!/usr/bin/env python3
"""Low-bit diagnostic screen for concatenated TriCube hash digests."""
...
def run_screen(implementations: list[str], samples: int, seed: int, message_len: int) -> list[dict[str, Any]]:
    ...
    rng = rng_for(seed ^ 0x10B17)
    for implementation in implementations:
        data = b"".join(
            hash_digest(implementation, message_from_seed(rng.getrandbits(64), message_len)) for _ in range(samples)
        )
        n_bytes = len(data)
        ...
        rows.append(
            {
                "screen": "hash-mode low-bit diagnostic screen",
                "implementation": implementation,
                "messages": samples,
                "message_bytes": message_len,
                "digest_bytes": n_bytes,
...
def main() -> int:
    ...
    write_hash_screen_outputs(args.out, "low_bit_hash_screen", rows, metadata)
    return 0
