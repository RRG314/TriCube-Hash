...
- `crypto_analysis/` contains reproducible development screens, low-bit
  diagnostics, hash-mode screens, the word-level schedule model, and optional
  external-tool setup checks.
...
Inside `crypto_analysis/`, the `screens/` subfolder contains stream-output
screen entry points. The `hash_mode/` subfolder contains the corresponding
digest-path screens. `run_all_screens.py` remains as the stream combined
runner, while `hash_mode/run_hash_screens.py` is the combined runner for
hash-mode checks.
